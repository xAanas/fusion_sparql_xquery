...
if(isset($_REQUEST['choixRelationSemantique'])){
$choixRelationSemantique = $_REQUEST['choixRelationSemantique'];
if ($choixRelationSemantique == 'oui') {
  if (isset($_REQUEST['relationsemantique'])) {
    $relationSemantiqueAExtraire = $_REQUEST['relationsemantique'];
  }
  else {
...
// récuperer le fichier.xml de recherche
  if(isset($_FILES['searchfile'])){
    ...
    if ($file_uploaded == 1) {
      if (isset($_REQUEST['similarityoption']))
        $similarityOption = $_REQUEST['similarityoption'];
      else
  ...
  echo "<h3> les requêtes executé </h3>";
  ...
  if ($construire_tableau_similarite > 0) {
    ...
    echo "<br/> no similarity calculated <br/>as de mots c";
  }
  ...
  if (\count($relationsemantique_for_json) > 0) {
    ...
    $resultat_json = array("nodes" => $fichier_xml_trouver,"edges" => "[]");
  }
  
  // ecrire le json dans un fichier json/xquery_json_result.json
  $xquery_json_result_file = fopen('json/xquery_json_result.json', 'r+');
