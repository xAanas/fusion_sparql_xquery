...
function settopic($file1, $file2) {
    ...
    $tab1=array();
    $i=0;
    for ($index = 0; $index < count($resultXml->contentMeta->genre); $index++) {
            $tab1[$i++]=$resultXml->contentMeta->genre[$index]->name;  
    }
    ...
    $i=0;
    for ($index = 0; $index < count($resultXml->contentMeta->genre); $index++) {
            $tab2[$i++]=$resultXml->contentMeta->genre[$index]->name;  
    }

    $score = count(array_intersect(array_map('strtolower', $tab1),array_map('strtolower', $tab2)));

    $fetchbyfilename = "select * from triplets_db.storylink_genre where file1='$file1' and file2='$file2'";
    $fetchbyfilenameInvers = "select * from triplets_db.storylink_genre where file1='$file2' and file2='$file1'";
    ...
    if (mysql_fetch_array($res) == false && mysql_fetch_array($res2) == false) {
        $insertscorefiles = "insert into triplets_db.storylink_genre (file1,file2,genre) value('$file1','$file2',$score)";
        $r = mysql_query($insertscorefiles);
    } else {
        $updatescorefiles = "update triplets_db.storylink_genre set genre=$score where file1='$file1' and file2='$file2'";
