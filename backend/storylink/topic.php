...
function settopic($file1, $file2) {
    ...
    for ($index = 0; $index < count($resultXml->contentMeta->subject); $index++) {
        if($resultXml->contentMeta->subject[$index]->attributes()->type=='cpnat:abstract'){
    ...
    $fetchbyfilename = "select * from triplets_db.storylink_topic where file1='$file1' and file2='$file2'";
    $fetchbyfilenameInvers = "select * from triplets_db.storylink_topic where file1='$file2' and file2='$file1'";
    ...
    if (mysql_fetch_array($res) == false && mysql_fetch_array($res2) == false) {
        $insertscorefiles = "insert into triplets_db.storylink_topic (file1,file2,topic) value('$file1','$file2',$score)";
        $r = mysql_query($insertscorefiles);
    } else {
        $updatescorefiles = "update triplets_db.storylink_topic set topic=$score where file1='$file1' and file2='$file2'";
